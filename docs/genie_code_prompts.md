...
**"¿Cómo empaquetamos esto con Databricks Asset Bundles para promover a Prod?"**

> Este prompt es oro si sale en Q&A — ellos deployan con DAB hoy. Les
> demuestra que Genie Code respeta su workflow.

```
Empaquetá todo el pipeline (bronze + silver + gold + metric views +
row filters) como un Databricks Asset Bundle:

- Generá el databricks.yml en la raíz
- Dos targets: dev y prod, con variables para catalog/schema y
  service_principal_id
- Incluí los recursos: el pipeline Lakeflow, el job de orquestación,
  las SQL warehouses necesarias
- Respetá el patrón de promoción Dev → PR → Main → deploy a Prod
- Agregá un README corto con el comando `databricks bundle deploy -t prod`

No incluyas el scaffold de data generation — eso solo corre en Dev.
```

**"¿Y si queremos que el Genie Space esté publicado en el workspace de negocio (Sandbox)?"**

```
Modificá la definición del Genie Space para que quede publicado en el
workspace Sandbox (no en Dev) y que el grupo `discover_domains_fraude`
tenga acceso de lectura. Agregá el snippet necesario para el bundle DAB
que haga este deploy cross-workspace.
```
